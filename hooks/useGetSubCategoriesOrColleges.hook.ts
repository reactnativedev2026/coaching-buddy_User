  ...
  useEffect(() => {
    ...
    (async function () {
      try {
        ...
        if (res.data != null) {
          if (res.data.isSubCategories && res.data.categories != null) {
            ...
            setCategories(data);
            setColleges([]);
          } else if (res.data.isColleges && res.data.colleges != null) {
            const data = res.data.colleges.map((item) => {
              const logoImages = item.images.logo?.map((i) => i.path) || [];
              const galleryImages =
                item.images.gallery?.map((i) => i.path) || [];
              const bannerImages = item.images.banner?.map((i) => i.path) || [];
              const coverImages = item.images.cover?.map((i) => i.path) || [];
